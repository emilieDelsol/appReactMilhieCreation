import React from "react";
const productsMedium = [
  {
    id: 1,
    title: "Arbre de vie sur agathe géode.",
    price: "45€",
    description: (
      <p>
        Arbre de vie fait main en fil de fer et perles d'améthyste le tout monté
        sur une Agathe Géode violette.
        <br />
        <br />
        Hauteur: 12.5cm
        <br />
        Largeur: 6cm
        <br />
        Poids: 200g
      </p>
    ),
    link: (
      <a
        href="https://www.vinted.fr/femmes/ensemble-de-bijoux/285486737-arbre-de-vie-sur-agathe-geode"
        title="fiche produit"
        target="blank"
      >
        Disponible...
        <br />
      </a>
    ),
    reference: "medium_LifeTree_01",
    images: [
      "/images/arbres/arbreM01.jpeg",
      "/images/arbres/arbreM01(01).jpeg",
      "/images/arbres/arbreM01(02).jpeg",
      "/images/arbres/arbreM01(03).jpeg",
      "/images/arbres/arbreM01(04).jpeg",
      "/images/arbres/arbreM01(05).jpeg",
      "/images/arbres/arbreM01(06).jpeg",
      "/images/arbres/arbreM01(07).jpeg",
      "/images/arbres/arbreM01(08).jpeg"
    ]
  },

  {
    id: 2,
    title: "Arbre de vie sur cristal de roche teinté.",
    price: "90€",
    description: (
      <p>
        Arbre de vie fait main en fil de fer et perles de rocaille le tout monté
        sur une géode de quartz de roche teinté.
        <br />
        <br />
        Hauteur: 11cm
        <br />
        Largeur: 7.5cm
      </p>
    ),
    link: (
      <a
        href="https://fr.shopping.rakuten.com/offer/buy/4804029395/arbre-de-vie-sur-amethyste.html"
        title="fiche produit"
        target="blank"
      >
        Sur commande...
        <br />
      </a>
    ),
    reference: "medium_life_tree_02",
    images: [
      "/images/arbres/arbreM02.jpeg",
      "/images/arbres/arbreM02(01).jpeg",
      "/images/arbres/arbreM02(02).jpeg",
      "/images/arbres/arbreM02(03).jpeg",
      "/images/arbres/arbreM02(04).jpeg",
      "/images/arbres/arbreM02(05).jpeg"
    ]
  },
  {
    id: 3,
    title: "Arbre de vie sur cristal de roche teinté.",
    price: "40€",
    description: (
      <p>
        Arbre de vie fait main en fil de fer et perles de rocaille le tout monté
        sur une géode de quartz de roche teinté.
        <br />
        <br />
        Hauteur: 11cm
        <br />
        Largeur: 7.5cm
      </p>
    ),

    link: (
      <a
        href="https://fr.shopping.rakuten.com/offer/buy/4804029395/arbre-de-vie-sur-amethyste.html"
        title="fiche produit"
        target="blank"
      >
        Sur commande...
        <br />
      </a>
    ),
    reference: "medium_life_tree_03",
    images: [
      "/images/arbres/arbreM03.jpeg",
      "/images/arbres/arbreM03(01).jpeg",
      "/images/arbres/arbreM03(02).jpeg",
      "/images/arbres/arbreM03(03).jpeg",
      "/images/arbres/arbreM03(04).jpeg",
      "/images/arbres/arbreM03(05).jpeg",
      "/images/arbres/arbreM03(06).jpeg"
    ]
  },

  {
    id: 4,
    title: "Arbre de vie sur améthyste.",
    price: "55€",
    description: (
      <p>
        Arbre de vie en fil de fer et perles d'améthyste fait main , monté sur
        une druse d'améthyste.
        <br />
        <br />
        Hauteur: 17 cm
        <br />
        Largeur: 6 cm
        <br />
        Poids: 338g
      </p>
    ),
    link: (
      <a
        href="https://fr.shopping.rakuten.com/offer/buy/4804029395/arbre-de-vie-sur-amethyste.html"
        title="fiche produit"
        target="blank"
      >
        Sur commande...
        <br />
      </a>
    ),
    reference: "medium_life_tree_4",
    images: [
      "/images/arbres/arbreM04.jpeg",
      "/images/arbres/arbreM04(01).jpeg",
      "/images/arbres/arbreM04(02).jpeg",
      "/images/arbres/arbreM04(03).jpeg"
    ]
  },

  {
    id: 5,
    title: "Arbre de vie sur cristal de roche teinté.",
    price: "40€",
    description: (
      <p>
        Arbre de vie fait main en fil de fer et perles de rocaille le tout monté
        sur une géode de cristal de roche teinté.
        <br />
        <br />
        Hauteur : 11cm
        <br />
        Largeur : 7cm
      </p>
    ),
    link: (
      <a
        href="https://fr.shopping.rakuten.com/offer/buy/4806967089/arbre-de-vie-sur-cristal-de-roche-teinte.html"
        target="blanck"
        title="pendentif"
      >
        Disponible...
        <br />
      </a>
    ),
    reference: "medium_life_tree_5",
    images: [
      "/images/arbres/arbreM05.jpeg",
      "/images/arbres/arbreM05(01).jpeg",
      "/images/arbres/arbreM05(02).jpeg",
      "/images/arbres/arbreM05(03).jpeg",
      "/images/arbres/arbreM05(04).jpeg",
      "/images/arbres/arbreM05(05).jpeg"
    ]
  },

  {
    id: 6,
    title: "Arbre de vie sur tranche de bois pétrifié.",
    price: "60€",
    description: (
      <p>
        Arbre de vie fait main en fil de fer et perles miyuki sur quartz brut .
        <br />
        <br />
        27 cm de hauteur
        <br />
        Poids: 644g
      </p>
    ),
    link: (
      <a
        href="https://fr.shopping.rakuten.com/offer/buy/4863359924/arbre-de-vie-bonsai-sur-tranche-de-bois-petrifie.html"
        target="blanck"
        title="arbre de vie sur tranche de bois pétrifié"
      >
        Disponible...
        <br />
      </a>
    ),
    reference: "medium_life_tree_6",
    images: [
      "/images/arbres/arbreM06.jpeg",
      "/images/arbres/arbreM06(01).jpeg",
      "/images/arbres/arbreM06(02).jpeg",
      "/images/arbres/arbreM06(03).jpeg",
      "/images/arbres/arbreM06(04).jpeg",
      "/images/arbres/arbreM06(05).jpeg",
      "/images/arbres/arbreM06(06).jpeg",
      "/images/arbres/arbreM06(07).jpeg",
      "/images/arbres/arbreM06(08).jpeg",
      "/images/arbres/arbreM06(09).jpeg"
    ]
  }
];

export default productsMedium;
